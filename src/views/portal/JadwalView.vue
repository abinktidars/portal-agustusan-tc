<template>
  <div>
    ...
    <div class="tcr-scroll chip-row">
      <button v-for="c in jadwalStore.cabangOptions" :key="c"
        class="chip" :class="{ active: jadwalStore.filterCabang === c }"
        @click="jadwalStore.filterCabang = c">{{ c }}</button>
    </div>
    <div class="tcr-scroll chip-row" style="padding:10px 0 6px;">
      <button v-for="s in jadwalStore.statusOptions" :key="s"
        class="chip" :class="{ active: jadwalStore.filterStatus === s }"
        @click="jadwalStore.filterStatus = s">{{ s }}</button>
    </div>
